...
@section('title', 'Nilai Non Tulis')

@section('breadcrumb', 'Nilai Non Tulis')
...
<style>
    ...
    /* PERUBAHAN: Kontras Tinggi untuk Form Input */
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Input Nilai Non Tulis</h3>
            ...
            <div class="card-body p-0 p-md-3">
                ...
                <div class="d-none d-md-block">
                    <form id="save-all-form" action="{{ route('wali.nilai_non_tulis.store_all') }}" method="POST">
                        @csrf
                            ...
                            <table class="table modern-table mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Nama Santri</th>
                                        <th>Muhafadzhoh</th>
                                        <th>Qiroatul Kutub</th>
                                        <th>Taftisyul Kutub</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $item->nama }}</td>
                                        <td>
                                            <select name="muhafadzhoh[{{ $item->id }}]" class="form-select form-select-sm border-0 bg-light">
                                                <option value="">-- Pilih --</option>
                                                <option value="ممتاز" {{ optional($nilaiNonTulis->get($item->id))->muhafadzhoh == 'ممتاز' ? 'selected' : '' }}>Mumtaz (ممتاز)</option>
                                                <option value="جيد" {{ optional($nilaiNonTulis->get($item->id))->muhafadzhoh == 'جيد' ? 'selected' : '' }}>Jayyid (جيد)</option>
                                                <option value="متوسط" {{ optional($nilaiNonTulis->get($item->id))->muhafadzhoh == 'متوسط' ? 'selected' : '' }}>Mutawwasith (متوسط)</option>
                                                <option value="رادئ" {{ optional($nilaiNonTulis->get($item->id))->muhafadzhoh == 'رادئ' ? 'selected' : '' }}>Rodi' (رادئ)</option>
                                            </select>
                                        </td>
                                        <td>
                                            <input type="number" name="qiroatul_kutub[{{ $item->id }}]" class="form-control form-control-sm border-0 bg-light text-center" 
                                                   value="{{ optional($nilaiNonTulis->get($item->id))->qiroatul_kutub ?? '' }}" 
                                                   min="0" max="100">
                                        </td>
                                        <td>
                                            <input type="number" name="taftisyul_kutub[{{ $item->id }}]" class="form-control form-control-sm border-0 bg-light text-center" 
                                                   value="{{ optional($nilaiNonTulis->get($item->id))->taftisyul_kutub ?? '' }}" 
                                                   min="0" max="100">
                                        </td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                @if($nilaiNonTulis->has($item->id))
                                                <button type="button" class="btn btn-sm btn-outline-danger rounded-pill btn-delete-individual ml-1" 
                                                        data-nilai-id="{{ $nilaiNonTulis->get($item->id)->id }}"
                                                        data-route="{{ route('wali.nilai_non_tulis.destroy', $nilaiNonTulis->get($item->id)) }}">
                ...
                <div class="d-md-none">
                    <form id="save-all-form-mobile" action="{{ route('wali.nilai_non_tulis.store_all') }}" method="POST">
                        @csrf
                        <div class="p-3">
                            @forelse($siswa as $index => $item)
                            <!-- Kartu Santri -->
                            <div class="student-card mb-4 p-4 bg-white rounded-lg shadow-sm border-0">
                                ...
                                <div class="form-group mb-3">
                                    <label class="small text-muted font-weight-light">Muhafadzhoh</label>
                                    <select name="muhafadzhoh[{{ $item->id }}]" class="form-select border-0 bg-light">
                                        <option value="">-- Pilih --</option>
                                        <option value="ممتاز" {{ optional($nilaiNonTulis->get($item->id))->muhafadzhoh == 'ممتاز' ? 'selected' : '' }}>Mumtaz (ممتاز)</option>
                                        <option value="جيد" {{ optional($nilaiNonTulis->get($item->id))->muhafadzhoh == 'جيد' ? 'selected' : '' }}>Jayyid (جيد)</option>
                                        <option value="متوسط" {{ optional($nilaiNonTulis->get($item->id))->muhafadzhoh == 'متوسط' ? 'selected' : '' }}>Mutawwasith (متوسط)</option>
                                ...
                                <div class="row">
                                    <div class="col-6 pr-2">
                                        <div class="form-group">
                                            <label class="small text-muted font-weight-light">Qiroatul Kutub</label>
                                            <input type="number" name="qiroatul_kutub[{{ $item->id }}]" class="form-control border-0 bg-light text-center" 
                                                   value="{{ optional($nilaiNonTulis->get($item->id))->qiroatul_kutub ?? '' }}" 
                                                   min="0" max="100">
                                        </div>
                                    </div>
                                    <div class="col-6 pl-2">
                                        <div class="form-group">
                                            <label class="small text-muted font-weight-light">Taftisyul Kutub</label>
                                            <input type="number" name="taftisyul_kutub[{{ $item->id }}]" class="form-control border-0 bg-light text-center" 
                                                   value="{{ optional($nilaiNonTulis->get($item->id))->taftisyul_kutub ?? '' }}" 
                                                   min="0" max="100">
                                        </div>
                                    </div>
...
<script>
 $(document).ready(function() {
    const routeStore = "{{ route('wali.nilai_non_tulis.store') }}";
    ...
    $(document).on('click', '.btn-save-individual', function() {
        ...
        const muhafadzhoh = container.find('select[name="muhafadzhoh[' + siswaId + ']"]').val();
        const qiroatul_kutub = container.find('input[name="qiroatul_kutub[' + siswaId + ']"]').val();
        const taftisyul_kutub = container.find('input[name="taftisyul_kutub[' + siswaId + ']"]').val();

        if (!muhafadzhoh && !qiroatul_kutub && !taftisyul_kutub) {
            Swal.fire({
                icon: 'warning',
                title: 'Data Kosong',
                text: 'Setidaknya satu nilai (Muhafadzhoh, Qiroatul Kutub, atau Taftisyul Kutub) harus diisi.',
        ...
        const formData = {
            _token: '{{ csrf_token() }}',
            siswa_id: siswaId,
            muhafadzhoh: muhafadzhoh,
            qiroatul_kutub: qiroatul_kutub,
            taftisyul_kutub: taftisyul_kutub,
        };
    ...
    $('#save-all-btn').click(function(e) {
        ...
        if (!hasAnyData) {
            Swal.fire({
                ...
                text: 'Tidak ada data nilai non tulis yang diisi. Tidak ada yang akan disimpan.',
                confirmButtonText: 'OK',
